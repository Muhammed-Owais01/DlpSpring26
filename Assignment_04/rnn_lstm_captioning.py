...
def rnn_backward(dh, cache):
    ...
    db = Wh.new_zeros(Wh.shape[0])

    dprev_h = dh.new_zeros(N, Wh.shape[0])
    ...
    def sample(self, images, max_length=15):
        ...
        for t in range(max_length):
            ...
            if self.cell_type == "rnn":
                ...
                next_h, prev_c = self.rnn.step_forward(word_embed, prev_h, prev_c, attn)
...
def dot_product_attention(prev_h, A):
    ...
    attn_scores = torch.bmm(A_flat.transpose(1, 2), prev_h.unsqueeze(2)).squeeze(2)
    attn_scores = attn_scores / math.sqrt(H)
    attn_weights_flat = F.softmax(attn_scores, dim=1)
